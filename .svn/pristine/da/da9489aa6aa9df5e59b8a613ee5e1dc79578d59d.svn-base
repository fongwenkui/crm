...
<SCRIPT language=javascript>
	...
	function deleteById(){
		...
		if(isDelete){
			window.location.href="${pageContext.request.contextPath }/CustomerAction_delete?cust_id=<s:property value="#cust.cust_id" />";
		}
	}
</SCRIPT>
										...
										<TABLE id=grid
											...
											<TBODY>
												...
												<TR 		
													...
													&nbsp;&nbsp;
													<a href="javaScript:void(0)" onclick='if(confirm("您是否要删除?"))window.location.href="${pageContext.request.contextPath }/CustomerAction_delete?cust_id=<s:property value="#cust.cust_id" />"'>删除</a>
													</TD>
													</s:if>
													<s:else>
														<input type="button" onclick="selectCustomer('<s:property value="#cust.cust_id" />',
																									'<s:property value="#cust.cust_name" />')" value="选择"/>
													</s:else>											
												</TR>
												</s:iterator>
